...
if
    (
    isset($_POST['f-grant-all-priv']) &&
    isset($_POST['f-cur-username-for-grant-all-priv']) &&
    isset($_POST['f-cur-hostname-for-grant-all-priv'])
    )
{   
    $_SESSION['s-cur-user'] = $_POST['f-cur-username-for-grant-all-priv'];
    $_SESSION['s-cur-host'] = $_POST['f-cur-hostname-for-grant-all-priv'];
    ...
    if 
       ...
       )
       {
        ...
        // call function 
        all_priv_grant($hostname, $username, $password, $curr_username, $curr_hostname);
       }
}
else {
...
function  all_priv_grant($hostname, $username, $password, $curr_username, $curr_hostname) {
    try {
        // establish connection
        $con = new mysqli($hostname, $username, $password);

        // Query
        $sql = "GRANT ALL ON *.* TO " . "'" . $curr_username . "'" . "@" . "'" .  $curr_hostname . "'" . ";";
        // run query
        try {
            $con->query($sql);
            // set SESSION variable with success status msg
            $_SESSION['s-g-a-p-status'] = "done";
            ...
            $_SESSION['s-g-a-p-status'] = "failed";
            $_SESSION['s-g-a-p-msg'] = $errorMsg;
