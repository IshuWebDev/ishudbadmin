...
if(isset($_GET['record_all']) &&
   ...
   isset($_SESSION['s-records-key-array'])
    ...
    $_SESSION['s-record-all'] = $_GET['record_all'];
    // store all values with ',' in local variable
    $idvals =  $_SESSION['s-record-all'];
    ...
    if (
       ...
       )
       {
        ...
        delete_record($hostname, $username, $password, $dbname, $tbname, $idvar, $idvals);
    }
}
else {
...
function delete_record($hostname, $username, $password, $dbname, $tbname, $idvar, $idvals) {
    try {
        // establish connection
        $con = new mysqli($hostname, $username, $password, $dbname);

        // Query
        $sql = "DELETE FROM" . " " .  $tbname . " " . "WHERE" . " " . $idvar . " " . "IN" . " " . "(" . $idvals . ");";
